...
export function withOverrides(WrappedComponent, componentName) {
  // eslint-disable-next-line flowtype/no-weak-types
  class WithOverrides extends React.Component {
    ...
    render() {
      ...
      const {
        forwardedRef
      } = this.props; // mergeOverrides from themeContext and props

      const overrides = mergeOverrides(this.context.overrides?.[componentName], this.props.overrides);
  ...
  _defineProperty(WithOverrides, "contextType", ThemeContext);
